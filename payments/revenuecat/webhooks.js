...
if (!REVENUECAT_WEBHOOK_SECRET) {
  throw new Error('❌ Missing REVENUECAT_WEBHOOK_SECRET in environment variables.');
}
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (!userId || !event) {
      return res.status(400).json({ success: false, error: 'Missing userId or event in payload.' });
    }
    ...
    if (!tier) {
      console.warn(`⚠️ No matching entitlement found for userId ${userId}. Event type: ${event.type}`);
      return res.status(200).json({ success: true, message: 'No valid entitlement found; no update performed.' });
    }
