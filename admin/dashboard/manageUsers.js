...
router.get('/reported', verifyAdmin, async (req, res) => {
  try {
    ...
    // TODO: Add pagination & filtering support for large result sets

    console.log(`[ADMIN] Retrieved ${users.length} reported users.`);

    res.status(200).json({ success: true, users });
  } catch (err) {
    console.error('🔥 [manageUsers.js] Error fetching reported users:', {
      message: err.message,
      stack: err.stack,
    });
...
router.post('/toggle-ban', verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: `User ${ban ? 'banned' : 'unbanned'} successfully.` });
  } catch (err) {
    console.error('🔥 [manageUsers.js] Error toggling user ban:', {
      message: err.message,
      stack: err.stack,
    });
